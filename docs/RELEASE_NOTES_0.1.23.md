...
- Added FCPXML timeline export from the Export panel, so Velorn edits can be imported into DaVinci Resolve, Final Cut Pro, and Premiere workflows for finishing.
...
- Export is currently one-way only: Velorn can export `.fcpxml`, but importing XML back into Velorn is not included yet.
- Exported XML includes media paths, clip timing, source trims, lanes, and Velorn clip metadata notes.
